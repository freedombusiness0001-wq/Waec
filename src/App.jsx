...
export function App(){
  ...
  const [events,setEvents]=useState({name:'',email:'',track:'Bootcamp'})
  ...
  const projects = [{t:'AgriTech',c:'AI'},{t:'FinPulse',c:'Fintech'},{t:'HealthMesh',c:'Health'},{t:'EduSpark',c:'AI'},{t:'PayGrid',c:'Fintech'},{t:'BioLens',c:'Health'}]

  useEffect(()=>{ const t=setTimeout(()=>setLoading(false),900); return ()=>clearTimeout(t)},[])
  useEffect(()=>{ localStorage.setItem('theme', dark?'dark':'light') },[dark])

  const submitNewsletter=(e)=>{ e.preventDefault(); setNewsletterMsg(`Subscribed: ${newsletter}`); setNewsletter('') }
  const submitEvent=(e)=>{ e.preventDefault(); setEventMsg(`Registration received for ${events.track}.`) }

  if(loading) return <div className="grid min-h-screen place-items-center bg-[#070b1a]"><div className="h-14 w-14 animate-spin rounded-full border-4 border-violet-300/30 border-t-violet-400"/></div>

  return <div className={`${dark?'bg-[#070b1a] text-white':'bg-white text-slate-900'} min-h-screen transition-colors`}>
    <div className="pointer-events-none fixed inset-0 -z-10 overflow-hidden"><div className="absolute -left-16 -top-16 h-72 w-72 rounded-full bg-violet-600/40 blur-[90px]"/><div className="absolute right-0 top-1/4 h-80 w-80 rounded-full bg-indigo-600/30 blur-[100px]"/></div>
    ...
    <section id="home" className="mx-auto grid min-h-[88vh] max-w-7xl items-center gap-10 px-5 py-20 lg:grid-cols-2">
      <motion.div {...fade}><p className="mb-4 inline-block rounded-full bg-gradient-to-r from-violet-600 to-indigo-600 px-4 py-1 text-xs">Innovation for Africa</p><h1 className="text-5xl font-black leading-tight md:text-7xl">Build. Launch. Scale.<br/><span className="bg-gradient-to-r from-violet-300 to-cyan-300 bg-clip-text text-transparent">Future-Ready Ventures</span></h1><p className="mt-6 max-w-xl text-slate-300">A premium innovation ecosystem for founders, creators, developers, and community changemakers.</p><div className="mt-8 flex flex-wrap gap-3"><button className="btn">Join Program</button><button className="btn2">Explore Projects</button></div></motion.div>
      <motion.div {...fade} className="glass p-6"><h3 className="text-lg font-bold">Innovation Dashboard</h3><div className="mt-4 grid gap-3 sm:grid-cols-2">{stats.map(s=><CounterCard key={s.k} {...s}/>)}</div></motion.div>
    </section>
    ...
    <section id="services" className="sec"><Wrap><H2>Services</H2><Grid cols={4}>{['Startup Incubation','AI & Robotics Lab','Digital Skills','SME Acceleration'].map(c=><Card key={c} title={c}/>)}</Grid></Wrap></section>
    ...
    <section id="events" className="sec"><Wrap><H2>Events</H2><form onSubmit={submitEvent} className="glass grid gap-3 p-6 md:grid-cols-3"><input className="input" placeholder="Full name" required value={events.name} onChange={e=>setEvents({...events,name:e.target.value})}/><input className="input" placeholder="Email" type="email" required value={events.email} onChange={e=>setEvents({...events,email:e.target.value})}/><select className="input" value={events.track} onChange={e=>setEvents({...events,track:e.target.value})}><option>Bootcamp</option><option>Hackathon</option><option>Demo Day</option></select><button className="btn w-fit">Register</button><p className="text-emerald-300 md:col-span-2">{eventMsg}</p></form></Wrap></section>
    ...
    <section className="sec"><Wrap><H2>Newsletter</H2><form onSubmit={submitNewsletter} className="glass mx-auto flex max-w-2xl flex-col gap-3 p-6 sm:flex-row"><input className="input flex-1" type="email" value={newsletter} onChange={e=>setNewsletter(e.target.value)} placeholder="Enter your email" required/><button className="btn">Subscribe</button></form><p className="mt-3 text-center text-emerald-300">{newsletterMsg}</p></Wrap></section>
...
function CounterCard({k,v}){const [n,setN]=useState(0); useEffect(()=>{let s=0;const inc=Math.ceil(v/40);const t=setInterval(()=>{s+=inc;if(s>=v){setN(v);clearInterval(t)}else setN(s)},30); return ()=>clearInterval(t)},[v]); return <div className="rounded-xl border border-white/15 bg-white/5 p-4"><p className="text-3xl font-black text-violet-300">{n.toLocaleString()}+</p><p className="mt-1 text-xs text-slate-300">{k}</p></div>}
